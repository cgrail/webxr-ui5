...
var mcTabs=new MCTabs();tinyMCEPopup.onInit.add(function(){var t=tinyMCEPopup.getWin().tinymce,d=tinyMCEPopup.dom,e=t.each;e(d.select('div.tabs'),function(b){d.setAttrib(b,"role","tablist");var i=tinyMCEPopup.dom.select('li',b);var c=function(a){mcTabs.displayTab(a,mcTabs.getPanelForTab(a));mcTabs.onChange.dispatch(a);};e(i,function(a){d.setAttrib(a,'role','tab');d.bind(a,'click',function(f){c(a.id);});});d.bind(d.getRoot(),'keydown',function(a){if(a.keyCode===9&&a.ctrlKey&&!a.altKey){t.dom.Event.cancel(a);}});e(d.select('a',b),function(a){d.setAttrib(a,'tabindex','-1');});});});
